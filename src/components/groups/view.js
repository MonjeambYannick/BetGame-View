...
export function View(props) {
    ...
    if(pagestate == 0)
    {
        ...
        return (
            ...
            <div className="view view-grouplist">
                ...
                <div className={"popup " + (btnstate.create || btnstate.join ? "show" : "")}>
                    {(btnstate.create==true ? (<Create />) : "")}
                    {(btnstate.join==true ? (<Join />) : "")}
                </div>

                <div className={"view-grouplist-list"}>
                {
                    apidata.data.map(obj => (
                        <Group name={obj.name} placement={obj.placement} api_id={obj.id}/>
                        ))
                    }
                </div>
            </div>
        ...
        return (
            <div className="view view-group">
                Group: (ID: {JSON.stringify(pagestate.data)})
            </div>
        )
    }
}